...
def filter_json_files(directory_path):
    ...
    Returns:
        tuple: (filtered_files, total_files, filtered_out_count)
    """
    ...
    if not os.path.exists(directory_path):
        ...
        return filtered_files, total_files, filtered_out_count
...
def main():
    ...
    # Get filtered files and statistics
    filtered_files, total_files, filtered_out_count = filter_json_files(directory_path)
    
    # Print statistics
    print(f"Total JSON files found: {total_files}")
    print(f"Files filtered out: {filtered_out_count}")
    print(f"Files remaining after filtering: {len(filtered_files)}")
    print(f"Filtering keywords: 已评审, 待评审, 关闭issue统计")
    
    # Print results
    print(f"\nFiles that don't match the filter criteria:")
